...
public class PublicFunction {
	...
	// URLのパラメータをkeyとvalueのMapに変換
	public static HashMap<String, String> getUrlToMap(String url) {
		HashMap<String, String> map = new HashMap<String, String>();
		List<NameValuePair> scmParam = new ArrayList<NameValuePair>();
		try {
			scmParam = URLEncodedUtils.parse(new URI(url), "UTF-8");
		} catch (URISyntaxException e) {
			e.printStackTrace();
			return map;
		}
		if (scmParam == null || scmParam.size() == 0) {
			Log.d("scmParam is empty...");
			return map;
		}
		for (NameValuePair param : scmParam) {
